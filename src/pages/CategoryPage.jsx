...
export default function CategoryPage({ 
  ...
  // Reset to page 1 when category, search, sort or shuffle changes
  useEffect(() => {
    setCurrentPage(1);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (!rawTerm) return rawCategoryProducts;

    const matched = [];
    for (const prod of rawCategoryProducts) {
  ...
  // Sort products (Diversifies brands when sortBy === 'featured', or relevance when searching)

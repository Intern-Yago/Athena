...
export default function RichTextEditor({
  ...
  const handleInsertProductLink = (prod) => {
    if (!prod) return;
    const textarea = textareaRef.current;
    const linkMarkdown = `[${prod.name}](/produto/${prod.slug || prod.id})`;
    
    if (textarea) {
      const { selectionStart, selectionEnd } = textarea;
      const textBefore = value.substring(0, selectionStart);
      const textAfter = value.substring(selectionEnd);
      const updatedValue = textBefore + linkMarkdown + textAfter;
      updateWithHistory(updatedValue, selectionStart + linkMarkdown.length);
    } else {
      updateWithHistory((value ? `${value} ` : '') + linkMarkdown);
    }

    if (onAddCompatibleProduct) {
      onAddCompatibleProduct(prod.id);
    }
  ...
  const handlePaste = (e) => {
    ...
    if (pastedText && (/\[20[01]~/.test(pastedText) || /\x1b/.test(pastedText))) {
      e.preventDefault();
      ...
      if (textarea) {
        ...
        const textBefore = value.substring(0, selectionStart);
        const textAfter = value.substring(selectionEnd);
        const updatedValue = textBefore + cleanText + textAfter;
        updateWithHistory(updatedValue, selectionStart + cleanText.length);
      }
    }
  };
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-2xs focus-within:border-amber-500 focus-within:ring-1 focus-within:ring-amber-500 transition-all">
        ...
        <div className="bg-slate-50/90 p-1.5 border-b border-slate-200 flex flex-wrap items-center gap-1 select-none text-xs">
          ...
          {products && products.length > 0 && (
            <button
              ...
              title="Vincular equipamento compatível com link do catálogo"
              className="px-2 py-1 rounded-lg hover:bg-amber-100/90 text-amber-900 border border-amber-300/80 bg-amber-50 text-[11px] font-bold flex items-center gap-1 transition-colors shadow-2xs cursor-pointer ml-1"
            >
              <LinkIcon className="w-3 h-3 text-amber-600" />
              <span>+ Link de Equipamento</span>
            </button>
          )}
        ...
        {activeTab === 'write' ? (
          ...
          <div className="p-4 bg-slate-50 min-h-[180px] text-xs">
            {value && value.trim() ? (
              <FormattedDescription text={value} products={products} />
            ) : (
      ...
      {activeTab === 'write' && value && (value.includes('**') || value.includes('*') || value.includes('•') || value.includes('1.') || value.includes('[cor=') || value.includes('[destaque]') || value.includes('[tamanho=') || value.includes('[size=') || value.includes('](')) && (
      ...
      {/* Modal for Inserting Product Link */}
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-xs font-extrabold uppercase tracking-wider text-slate-800">Inserir Link de Equipamento</h4>
                  <p className="text-[10px] text-slate-500">Selecione o produto para criar o link de compatibilidade</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="p-4 space-y-3">
              <div className="relative">
                <input
                  type="text"
                  autoFocus
                  placeholder="Buscar produto por nome, modelo ou código..."
                  value={productLinkSearch}
                  onChange={(e) => setProductLinkSearch(e.target.value)}
                  className="w-full pl-9 pr-4 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs font-bold text-slate-800 placeholder:text-slate-400 focus:bg-white focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none"
                />
              ...
              <div className="max-h-60 overflow-y-auto space-y-1.5 pr-1">
                {(() => {
                  const filtered = (products || []).filter(p => {
                    if (!productLinkSearch.trim()) return true;
                    const query = productLinkSearch.toLowerCase();
                    return (
                  ...
                  if (filtered.length === 0) {
                    return (
                      <p className="text-center text-xs text-slate-400 py-6 italic">
                        Nenhum produto encontrado para "{productLinkSearch}".
                      </p>
                    );
                  }
                  ...
                  return filtered.map(prod => (
                    <button
                      key={prod.id}
                      type="button"
                      onClick={() => handleInsertProductLink(prod)}
                      className="w-full p-2.5 rounded-xl border border-slate-100 hover:border-amber-400 hover:bg-amber-50/60 text-left transition flex items-center justify-between gap-3 group cursor-pointer"
                    >
                      ...
                      <span className="px-2 py-1 rounded-md bg-white border border-slate-200 group-hover:border-amber-300 text-slate-600 group-hover:text-amber-900 text-[10px] font-extrabold shrink-0">
                        Inserir Link ↵
                      </span>
                    </button>
                  ));
            ...
            <div className="p-3 bg-slate-50 border-t border-slate-200 flex justify-end">
              <button
                type="button"
                onClick={() => setIsProductLinkModalOpen(false)}
                className="px-3 py-1.5 rounded-lg border border-slate-200 bg-white text-xs font-bold text-slate-700 hover:bg-slate-100 cursor-pointer"
              >
                Cancelar
              </button>
            </div>
